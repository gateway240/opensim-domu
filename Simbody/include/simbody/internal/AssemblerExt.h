#ifndef SimTK_SIMBODY_ASSEMBLER_EXT_H_
#define SimTK_SIMBODY_ASSEMBLER_EXT_H_
 ...
 *                                                                            *
 * Portions copyright (c) 2010-24 Stanford University and the Authors.        *
...
AssemblerExt also supports high-performance repeated assembly (also known as
...
satisfied initially, while AssemblerExt will attempt to satisfy them regardless, 
and may take a series of increasingly desperate measures to do so. 

<h2>Basic assembly:</h2>
This is the most common use of the AssemblerExt: modify a System's given State
...
@code
  ...
  AssemblerExt assembler(system); // construct the AssemblerExt study object
...
Allowable (gradual) changes between tracking frames are:
  ...
  - Any changes allowed by the assembly conditions currently included in this
    AssemblerExt, for example marker location observations for the Markers
    assembly condition.
...
**/
class SimTK_SIMBODY_EXPORT AssemblerExt : public Study {
...
/** AssemblerExt::FreeQIndex is a unique integer type used for accessing
the subset of q's that the AssemblerExt is permitted to change. **/
SimTK_DEFINE_UNIQUE_LOCAL_INDEX_TYPE(AssemblerExt,FreeQIndex);
...
/*@{*/
/** Create an AssemblerExt study for the given MultibodySystem. The
AssemblerExt's current state is set to the System's default state but with
Euler angles used instead of quaternions. **/
explicit AssemblerExt(const MultibodySystem& system);
...
behavior. **/
AssemblerExt& setErrorTolerance(Real tolerance=0) {
    SimTK_ERRCHK1_ALWAYS(0 <= tolerance,
        "AssemblerExt::setTolerance()", "The requested error tolerance %g"
...
AssemblerExt& setAccuracy(Real accuracy=0) {
    SimTK_ERRCHK2_ALWAYS(0 <= accuracy && accuracy < 1,
        "AssemblerExt::setAccuracy()", "The requested accuracy %g is illegal;"
...
AssemblerExt& setSystemConstraintsWeight(Real weight)
...
AssemblerExt& setAssemblyConditionWeight(AssemblyConditionIndex condition, 
                                      Real                   weight) {
    SimTK_INDEXCHECK_ALWAYS(condition, conditions.size(),
        "AssemblerExt::setAssemblyConditionWeight()");
    SimTK_ERRCHK1_ALWAYS(weight >= 0, "AssemblerExt::setAssemblyConditionWeight()",
...
Real getAssemblyConditionWeight(AssemblyConditionIndex condition) const {
    SimTK_INDEXCHECK_ALWAYS(condition, conditions.size(),
        "AssemblerExt::getAssemblyConditionWeight()");
    return weights[condition];
}

/** Add an assembly error condition to this AssemblerExt study, taking over 
...
/** Add an assembly goal to this AssemblerExt study, taking over ownership
...
/** Set the AssemblerExt's internal state from an existing state which must
be suitable for use with the AssemblerExt's System as supplied at the time
the AssemblerExt was constructed. All variables are copied, not just q's, so
the AssemblerExt must be reinitialized after this call in case modeling 
options, instance variables, or time have changed. **/
AssemblerExt& setInternalState(const State& state) {
    uninitialize();
    getMatterSubsystem().convertToEulerAngles(state, internalState);
    system.realizeModel(internalState);
    return *this;
}
/** Initialize the AssemblerExt to prepare for performing assembly analysis.
...
assemble() instead of track(). See the AssemblerExt class documentation for 
...
/** Given an existing State that is suitable for the AssemblerExt's System, 
update its q's from those found in the AssemblerExt's internal State, leaving 
...
{   SimTK_ERRCHK2_ALWAYS(lowerBound <= upperBound, "AssemblerExt::restrictQ()", 
...
SimTK::Matrix calcCurrentJacobian(SimTK::State &s) const;
SimTK::Vector calcCurrentGradient(SimTK::State &s) const;

/** @name                    Statistics
The AssemblerExt keeps counters of various internal operations it performs
...
AssemblerExt was created or the most recent resetStats() call. **/
...
/** Uninitialize the AssemblerExt. After this call the AssemblerExt must be
...
/** Check whether the AssemblerExt has been initialized since the last change
was made to its contents. **/
bool isInitialized() const {return alreadyInitialized;}

/** This provides read-only access to the AssemblerExt's internal State; you
...
else and persist throughout the lifetime of this AssemblerExt object. **/
...
AssemblerExt (that is, the System that was supplied in the AssemblerExt's
constructor. **/
const MultibodySystem& getMultibodySystem() const 
{   return system; }
/** Return a reference to the SimbodyMatterSubsystem that is contained
in the MultibodySystem that is associated with this AssemblerExt. **/
const SimbodyMatterSubsystem& getMatterSubsystem() const
{   return system.getMatterSubsystem(); }
/*@}*/

/** Destruct the AssemblerExt objects and any Assembly Condition objects it
contains. **/
~AssemblerExt();
...
// These are filled in when the AssemblerExt is initialized.
...
class AssemblerExtSystem; // local class
mutable AssemblerExtSystem* asmSys;
...
friend class AssemblerExtSystem;
};

} // namespace SimTK

#endif // SimTK_SIMBODY_ASSEMBLER_EXT_H_
