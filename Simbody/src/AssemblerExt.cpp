...
#include "simbody/internal/AssemblerExt.h"
#include "simbody/internal/AssemblyConditionExt.h"
#include <map>
#include <iostream>
...
//                               EXCEPTIONS
//------------------------------------------------------------------------------
class AssemblerExt::AssembleFailed : public Exception::Base {
public:
    AssembleFailed
    ...
    {
        setMessage(
            "Method AssemblerExt::assemble() failed because:\n" + String(why)
            + "\nAssembly error tolerance achieved: "
            + String(tolAchieved) + " required: " + String(tolRequired)
            + ".");
    }
};
class AssemblerExt::TrackFailed : public Exception::Base {
public:
    TrackFailed
       (const char* fn, int ln, const char* why, 
        Real tolAchieved, Real tolRequired) 
    ...
    {
        setMessage(
            "Method AssemblerExt::track() failed because:\n" + String(why)
            + "\nAssembly error tolerance achieved: "
            + String(tolAchieved) + " required: " + String(tolRequired)
            + ".");
    }
...
public:
    ...
    int calcErrorJacobian(const State& state, Matrix& jacobian) const override {
        ...
        if (np == nq) {
            ...
            for (AssemblerExt::FreeQIndex fx(0); fx < np; ++fx)
...
class AssemblerExt::AssemblerExtSystem : public OptimizerSystem {
public:
    AssemblerExtSystem(AssemblerExt& assembler)
    ...
    Real calcCurrentGoal() const {
        ...
        SimTK_ERRCHK1_ALWAYS(status==0, 
            "AssemblerExtSystem::calcCurrentGoal()",
    ...
    Vector calcCurrentGradient() const {
        ...
        SimTK_ERRCHK1_ALWAYS(status==0, 
            "AssemblerExtSystem::calcCurrentGradient()",
    ...
    Vector calcCurrentErrors() const {
        ...
        SimTK_ERRCHK1_ALWAYS(status==0, 
            "AssemblerExtSystem::calcCurrentErrors()",
    ...
    Matrix calcCurrentJacobian() const {
        ...
        SimTK_ERRCHK1_ALWAYS(status==0, 
            "AssemblerExtSystem::calcCurrentJacobian()",
    ...
    {   ++nEvalObjective;
        ...
        for (unsigned i=0; i < assembler.goals.size(); ++i) {
            ...
            SimTK_ERRCHK2_ALWAYS(goalValue >= 0,
                "AssemblerExtSystem::objectiveFunc()",
    ...
    public:
        NumGradientFunc(AssemblerExt& assembler,
    ...
    public:
        NumJacobianFunc(AssemblerExt& assembler,
...
//                                 ASSEMBLER
//------------------------------------------------------------------------------
AssemblerExt::AssemblerExt(const MultibodySystem& system)
...
AssemblyConditionIndex AssemblerExt::
...
adoptAssemblyGoal(AssemblyCondition* p, Real weight) {
    SimTK_ERRCHK_ALWAYS(p != 0, "AssemblerExt::adoptAssemblyGoal()",
        "Null assembly condition pointer.");
    SimTK_ERRCHK1_ALWAYS(weight >= 0, "AssemblerExt::adoptAssemblyGoal()",
...
void AssemblerExt::initialize() const {
    ...
    /*NOTREACHED*/
    // TODO: This currently unused code would allow the AssemblerExt to lock out 
...
void AssemblerExt::reinitializeWithExtraQsLocked
   (const Array_<QIndex>& toBeLocked) const {
         ...
         p != userLockedQs.end(); ++p)
    {
        ...
        for (QSet::const_iterator qp = qs.begin(); qp != qs.end(); ++qp) {
            ...
            SimTK_ERRCHK3_ALWAYS(q < nq, "AssemblerExt::initialize()", 
    ...
    // Allocate an AssemblerExtSystem which is in the form of an objective
    // function for the SimTK::Optimizer class.
    asmSys = new AssemblerExtSystem(*const_cast<AssemblerExt*>(this));
...
void AssemblerExt::uninitialize() const {
    if (!alreadyInitialized)
        return;
...
Real AssemblerExt::track(Real frameTime) {
    initialize();
    ++nAssemblySteps;
    ...
    // START CUSTOM KF CODE
     // Optimize
    Vector freeQs = getFreeQsFromInternalState();
    // std::cout << freeQs << std::endl;
    optimizer->setConvergenceTolerance(getAccuracyInUse());
    optimizer->setConstraintTolerance(getErrorToleranceInUse());
    try
    {   optimizer->optimize(freeQs); }
...
SimTK::Matrix AssemblerExt::calcCurrentJacobian(SimTK::State &s) const {
    return asmSys->calcCurrentJacobian();
}
SimTK::Vector AssemblerExt::calcCurrentGradient(SimTK::State &s) const {
    return asmSys->calcCurrentGradient();
}
int AssemblerExt::getNumGoalEvals()  const 
{   return asmSys ? asmSys->getNumObjectiveEvals() : 0;}
int AssemblerExt::getNumErrorEvals() const
{   return asmSys ? asmSys->getNumConstraintEvals() : 0;}
int AssemblerExt::getNumGoalGradientEvals()   const
{   return asmSys ? asmSys->getNumGradientEvals() : 0;}
int AssemblerExt::getNumErrorJacobianEvals()   const
{   return asmSys ? asmSys->getNumJacobianEvals() : 0;}
int AssemblerExt::getNumAssemblySteps() const
{   return nAssemblySteps; }
int AssemblerExt::getNumInitializations() const
{   return nInitializations; }
void AssemblerExt::resetStats() const
