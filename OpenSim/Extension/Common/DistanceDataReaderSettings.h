#ifndef OPENSIM_DISTANCE_DATA_READER_SETTINGS_H_
#define OPENSIM_DISTANCE_DATA_READER_SETTINGS_H_
/* -------------------------------------------------------------------------- *
 *                          OpenSim:  DistanceDataReaderSettings.h               *
 * -------------------------------------------------------------------------- *
...
/** DistanceDataReaderSettings is a class that reads files produced by IMU
   manufacturer Xsens and produces datatables from them. This is intended to
   help consume IMU outputs.*/
class OSIMCOMMON_API DistanceDataReaderSettings : public Object {
OpenSim_DECLARE_CONCRETE_OBJECT(DistanceDataReaderSettings, Object);
public:
...
public:
    // Default Constructor
    DistanceDataReaderSettings() {
        constructProperties();
    };
    // Constructor from XML file
    DistanceDataReaderSettings(const std::string& xmlFile) : Object(xmlFile, false) {
        constructProperties();
        updateFromXMLDocument();
    };

    virtual ~DistanceDataReaderSettings() = default;
