#ifndef OPENSIM_ASSEMBLY_SOLVER_EXT_H_
#define OPENSIM_ASSEMBLY_SOLVER_EXT_H_
/* -------------------------------------------------------------------------- *
 *                         OpenSim:  AssemblySolverExt.h                      *
 * -------------------------------------------------------------------------- *
 ...
 *
 * @author Alexander Beattie, Ajay Seth
 * @version 1.0
 */
class OSIMSIMULATION_API AssemblySolverExt: public Solver {
OpenSim_DECLARE_CONCRETE_OBJECT(AssemblySolverExt, Solver);
//=============================================================================
// METHODS
//=============================================================================
public:
    ...
    /** %Set the unitless accuracy of the assembly solution, which dictates
        ...
        Note, setting the accuracy will invalidate the AssemblySolverExt and one
    ...
    // Reference https://github.com/opensim-org/opensim-core/compare/main...antoinefalisse:opensim-core:KS
    virtual void setState(SimTK::State &s);

    SimTK::Matrix calcCurrentJacobian(SimTK::State &s);
    SimTK::Vector calcCurrentGradient(SimTK::State &s);

    /** Read access to the underlying SimTK::Assembler. */
    const SimTK::AssemblerExt& getAssembler() const;
...
protected:
    ...
    SimTK::AssemblerExt& updAssembler();
...
private:
    ...
    SimTK::ResetOnCopy< std::unique_ptr<SimTK::AssemblerExt>> _assembler;

    SimTK::Array_<SimTK::QValue*> _coordinateAssemblyConditions;
//=============================================================================
};  // END of class AssemblySolverExt
//=============================================================================
} // namespace

#endif // OPENSIM_ASSEMBLY_SOLVER_EXT_H_
