 ...
 */
InverseKinematicsSolverExt::InverseKinematicsSolverExt(
    const Model &model, std::shared_ptr<MarkersReference> markersReference,
    SimTK::Array_<CoordinateReference> &coordinateReferences,
    double constraintWeight)
    : InverseKinematicsSolverExt(model, markersReference, nullptr, nullptr,
                                 coordinateReferences,
                                 constraintWeight = SimTK::Infinity) {}

InverseKinematicsSolverExt::InverseKinematicsSolverExt(
    const Model &model, std::shared_ptr<DistancesReference> distancesReference,
    SimTK::Array_<CoordinateReference> &coordinateReferences,
    double constraintWeight)
...
InverseKinematicsSolverExt::InverseKinematicsSolverExt(
  ...
  if (_markersReference && _markersReference->getNumRefs() > 0) {
    ...
    if (markerNames.size() < 1) {
      log_error("InverseKinematicsSolverExt: No markers available from data "
                "provided.");
      throw Exception("InverseKinematicsSolverExt: No markers available from "
                      "data provided.");
    }
    int index = 0, cnt = 0;
    ...
    if (cnt < 1) {
      log_error("InverseKinematicsSolverExt: Marker data does not correspond "
                "to any model markers.");
      throw Exception("InverseKinematicsSolverExt: Marker data does not "
                      "correspond to any model markers.");
    }
    if (cnt < 4)
      log_warn(
          "WARNING: InverseKinematicsSolverExt found only {} markers to track.",
          cnt);
  }
  if (_orientationsReference && _orientationsReference->getNumRefs() > 0) {
    ...
    if (sensorNames.size() < 1) {
      log_error("InverseKinematicsSolverExt: No orientation data is available "
                "from provided "
                "source.");
      throw Exception(
          "InverseKinematicsSolverExt: No orientation data is available "
    ...
    for (const auto &modelFrame : onFrames) {
      ...
      auto found =
          std::find(sensorNames.begin(), sensorNames.end(), modelFrameName);
      if (found != sensorNames.end()) {
        cnt++;
      }
    }
    ...
    log_info("InverseKinematicsSolverExt found {} model frames to track.", cnt);
  }
  ...
  if (_distancesReference && _distancesReference->getNumRefs() > 0) {
    ...
    if (sensorNames.size() < 1) {
      log_error("InverseKinematicsSolverExt: No distance data is available "
                "from provided "
                "source.");
      throw Exception(
          "InverseKinematicsSolverExt: No distance data is available "
          "from the provided source.");
    }
    int cnt = 0;
    const auto onFrames = getModel().getComponentList<PhysicalFrame>();
...
/* Change the weighting of a marker to take effect when assemble or track is
   called next. Update a marker's weight by name. */
void InverseKinematicsSolverExt::updateMarkerWeight(
    const std::string &markerName, double value) {
  const Array_<std::string> &names = _markersReference->getNames();
  SimTK::Array_<const std::string>::iterator p =
      std::find(names.begin(), names.end(), markerName);
  int index = (int)std::distance(names.begin(), p);
  updateMarkerWeight(index, value);
}

/* Update a marker's weight by its index. */
void InverseKinematicsSolverExt::updateMarkerWeight(int markerIndex,
                                                    double value) {
  if (markerIndex >= 0 &&
      markerIndex < _markersReference->getMarkerWeightSet().getSize()) {
    // update the solver's copy of the reference
    _markersReference->updMarkerWeightSet()[markerIndex].setWeight(value);
    _markerAssemblyCondition->changeMarkerWeight(
        SimTK::Markers::MarkerIx(markerIndex), value);
  } else
    throw Exception(
        "InverseKinematicsSolverExt::updateMarkerWeight: invalid markerIndex.");
}

/* Update all markers weights by order in the markersReference passed in to
   construct the solver. */
void InverseKinematicsSolverExt::updateMarkerWeights(
    const SimTK::Array_<double> &weights) {
  if (static_cast<unsigned>(
          _markersReference->getMarkerWeightSet().getSize()) ==
      weights.size()) {
    for (unsigned int i = 0; i < weights.size(); i++) {
      _markersReference->updMarkerWeightSet()[i].setWeight(weights[i]);
      _markerAssemblyCondition->changeMarkerWeight(SimTK::Markers::MarkerIx(i),
                                                   weights[i]);
    }
  } else
    throw Exception("InverseKinematicsSolverExt::updateMarkerWeights: invalid "
                    "size of weights.");
}
...
void InverseKinematicsSolverExt::updateOrientationWeight(
    const std::string &orientationName, double value) {
  const Array_<std::string> &names = _orientationsReference->getNames();
  SimTK::Array_<const std::string>::iterator p =
      std::find(names.begin(), names.end(), orientationName);
  int index = (int)std::distance(names.begin(), p);
  updateOrientationWeight(index, value);
}

/* Update an orientation sensor's weight by its index. */
void InverseKinematicsSolverExt::updateOrientationWeight(int orientationIndex,
                                                         double value) {
  if (orientationIndex >= 0 &&
      orientationIndex <
          _orientationsReference->updOrientationWeightSet().getSize()) {
    _orientationsReference->updOrientationWeightSet()[orientationIndex]
        .setWeight(value);
    _orientationAssemblyCondition->changeOSensorWeight(
        SimTK::OrientationSensors::OSensorIx(orientationIndex), value);
  } else
    throw Exception("InverseKinematicsSolverExt::updateOrientationWeight: "
                    "invalid orientationIndex.");
}

/* Update all orientation sensor weights by order in the orientationsReference
passed in to construct the solver. */
void InverseKinematicsSolverExt::updateOrientationWeights(
    const SimTK::Array_<double> &weights) {
  if (static_cast<unsigned>(
          _orientationsReference->updOrientationWeightSet().getSize()) ==
      weights.size()) {
    for (unsigned int i = 0; i < weights.size(); i++) {
      _orientationsReference->updOrientationWeightSet()[i].setWeight(
          weights[i]);
      _orientationAssemblyCondition->changeOSensorWeight(
          SimTK::OrientationSensors::OSensorIx(i), weights[i]);
    }
  } else
    throw Exception("InverseKinematicsSolverExt::updateOrientationWeights: "
                    "invalid size of weights.");
}

/* Compute and return the spatial location of a marker in ground. */
SimTK::Vec3 InverseKinematicsSolverExt::computeCurrentMarkerLocation(
...
SimTK::Vec3
InverseKinematicsSolverExt::computeCurrentMarkerLocation(int markerIndex) {
  if (markerIndex >= 0 &&
      markerIndex < _markerAssemblyCondition->getNumMarkers()) {
    return _markerAssemblyCondition->findCurrentMarkerLocation(
        SimTK::Markers::MarkerIx(markerIndex));
  } else
    throw Exception("InverseKinematicsSolverExt::computeCurrentMarkerLocation: "
                    "invalid markerIndex.");
}

/* Compute and return the spatial locations of all markers in ground. */
void InverseKinematicsSolverExt::computeCurrentMarkerLocations(
  ...
  for (unsigned int i = 0; i < markerLocations.size(); i++)
    markerLocations[i] = _markerAssemblyCondition->findCurrentMarkerLocation(
        SimTK::Markers::MarkerIx(i));
}

/* Compute and return the distance error between model marker and observation.
 */
double InverseKinematicsSolverExt::computeCurrentMarkerError(
    const std::string &markerName) {
  const Array_<std::string> &names = _markersReference->getNames();
  SimTK::Array_<const std::string>::iterator p =
      std::find(names.begin(), names.end(), markerName);
...
double InverseKinematicsSolverExt::computeCurrentMarkerError(int markerIndex) {
  if (markerIndex >= 0 &&
      markerIndex < _markerAssemblyCondition->getNumMarkers()) {
    return _markerAssemblyCondition->findCurrentMarkerError(
        SimTK::Markers::MarkerIx(markerIndex));
  } else
    throw Exception("InverseKinematicsSolverExt::computeCurrentMarkerError: "
                    "invalid markerIndex.");
}

/* Compute and return the distance errors between all model markers and their
 * observations. */
void InverseKinematicsSolverExt::computeCurrentMarkerErrors(
  ...
  for (unsigned int i = 0; i < markerErrors.size(); i++)
    markerErrors[i] = _markerAssemblyCondition->findCurrentMarkerError(
        SimTK::Markers::MarkerIx(i));
}

/* Compute and return the squared-distance error between model marker and
 * observation. */
double InverseKinematicsSolverExt::computeCurrentSquaredMarkerError(
    const std::string &markerName) {
  const Array_<std::string> &names = _markersReference->getNames();
  SimTK::Array_<const std::string>::iterator p =
      std::find(names.begin(), names.end(), markerName);
...
double
InverseKinematicsSolverExt::computeCurrentSquaredMarkerError(int markerIndex) {
  if (markerIndex >= 0 &&
      markerIndex < _markerAssemblyCondition->getNumMarkers()) {
    return _markerAssemblyCondition->findCurrentMarkerErrorSquared(
        SimTK::Markers::MarkerIx(markerIndex));
  } else
    throw Exception("InverseKinematicsSolverExt::"
                    "computeCurrentMarkerSquaredError: invalid markerIndex.");
}

/* Compute and return the distance errors between all model marker and
 * observations. */
void InverseKinematicsSolverExt::computeCurrentSquaredMarkerErrors(
    SimTK::Array_<double> &markerErrors) {
  markerErrors.resize(_markerAssemblyCondition->getNumMarkers());
  for (unsigned int i = 0; i < markerErrors.size(); i++)
    markerErrors[i] = _markerAssemblyCondition->findCurrentMarkerErrorSquared(
        SimTK::Markers::MarkerIx(i));
}

/* Marker errors are reported in order different from tasks file or model, find
 * name corresponding to passed in index  */
std::string
InverseKinematicsSolverExt::getMarkerNameForIndex(int markerIndex) const {
  return _markerAssemblyCondition->getMarkerName(
      SimTK::Markers::MarkerIx(markerIndex));
}

/* Compute and return the spatial orientation of an o-sensor in ground. */
SimTK::Rotation InverseKinematicsSolverExt::computeCurrentSensorOrientation(
    const std::string &osensorName) {
  const Array_<std::string> &names = _orientationsReference->getNames();
  SimTK::Array_<const std::string>::iterator p =
      std::find(names.begin(), names.end(), osensorName);
  int index = (int)std::distance(names.begin(), p);
  return computeCurrentSensorOrientation(index);
}

SimTK::Rotation
InverseKinematicsSolverExt::computeCurrentSensorOrientation(int osensorIndex) {
  if (osensorIndex >= 0 &&
      osensorIndex < _orientationAssemblyCondition->getNumOSensors()) {
    return _orientationAssemblyCondition->findCurrentOSensorOrientation(
        SimTK::OrientationSensors::OSensorIx(osensorIndex));
  } else
    throw Exception("InverseKinematicsSolverExt::"
                    "computeCurrentOSensorOrientation: invalid osensorIndex.");
}
...
void InverseKinematicsSolverExt::computeCurrentSensorOrientations(
  ...
  for (unsigned int i = 0; i < osensorOrientations.size(); i++)
    osensorOrientations[i] =
        _orientationAssemblyCondition->findCurrentOSensorOrientation(
            SimTK::OrientationSensors::OSensorIx(i));
}

/* Compute and return the orientation error between model o-sensor and
 * observation. */
double InverseKinematicsSolverExt::computeCurrentOrientationError(
    const std::string &osensorName) {
  const Array_<std::string> &names = _orientationsReference->getNames();
  SimTK::Array_<const std::string>::iterator p =
      std::find(names.begin(), names.end(), osensorName);
...
double
InverseKinematicsSolverExt::computeCurrentOrientationError(int osensorIndex) {
  if (osensorIndex >= 0 &&
      osensorIndex < _orientationAssemblyCondition->getNumOSensors()) {
    return _orientationAssemblyCondition->findCurrentOSensorError(
        SimTK::OrientationSensors::OSensorIx(osensorIndex));
  } else
    throw Exception(
...
/* Compute and return the distance between all model o-sensors and their
 * observations. */
void InverseKinematicsSolverExt::computeCurrentOrientationErrors(
  ...
  for (unsigned int i = 0; i < osensorErrors.size(); i++)
    osensorErrors[i] = _orientationAssemblyCondition->findCurrentOSensorError(
        OrientationSensors::OSensorIx(i));
}
...
std::string InverseKinematicsSolverExt::getOrientationSensorNameForIndex(
    int osensorIndex) const {
  return _orientationAssemblyCondition->getOSensorName(
      SimTK::OrientationSensors::OSensorIx(osensorIndex));
}

// START DISTANCES
/* Compute and return the spatial orientation of an o-sensor in ground. */
SimTK::Real InverseKinematicsSolverExt::computeCurrentSensorDistance(
    const std::string &dsesnorName) {
  const Array_<std::string> &names = _distancesReference->getNames();
  SimTK::Array_<const std::string>::iterator p =
      std::find(names.begin(), names.end(), dsesnorName);
  int index = (int)std::distance(names.begin(), p);
  return computeCurrentSensorDistance(index);
}

SimTK::Real
InverseKinematicsSolverExt::computeCurrentSensorDistance(int dsensorIndex) {
  if (dsensorIndex >= 0 &&
      dsensorIndex < _distanceAssemblyCondition->getNumDSensors()) {
    return _distanceAssemblyCondition->findCurrentDSensorDistance(
        SimTK::DistanceSensors::DSensorIx(dsensorIndex));
  } else
    throw Exception("InverseKinematicsSolverExt::computeCurrentDSensorDistance:"
                    " invalid dsensorIndex.");
}
...
void InverseKinematicsSolverExt::computeCurrentSensorDistances(
  ...
  for (unsigned int i = 0; i < dsensorDistances.size(); i++)
    dsensorDistances[i] =
        _distanceAssemblyCondition->findCurrentDSensorDistance(
            SimTK::DistanceSensors::DSensorIx(i));
}

/* Compute and return the orientation error between model o-sensor and
 * observation. */
double InverseKinematicsSolverExt::computeCurrentDistanceError(
    const std::string &dsensorName) {
  const Array_<std::string> &names = _distancesReference->getNames();
  SimTK::Array_<const std::string>::iterator p =
      std::find(names.begin(), names.end(), dsensorName);
  int index = (int)std::distance(names.begin(), p);
  return computeCurrentDistanceError(index);
}

double
InverseKinematicsSolverExt::computeCurrentDistanceError(int dsensorIndex) {
  if (dsensorIndex >= 0 &&
      dsensorIndex < _distanceAssemblyCondition->getNumDSensors()) {
    return _distanceAssemblyCondition->findCurrentDSensorError(
        SimTK::DistanceSensors::DSensorIx(dsensorIndex));
  } else
    throw Exception("InverseKinematicsSolverExt::computeCurrentDistanceError: "
                    "invalid sensor Index.");
}
...
void InverseKinematicsSolverExt::computeCurrentDistanceErrors(
  ...
  for (unsigned int i = 0; i < dsensorErrors.size(); i++)
    dsensorErrors[i] = _distanceAssemblyCondition->findCurrentDSensorError(
        DistanceSensors::DSensorIx(i));
}

/* Distance errors may be reported in an order that may be different from
   tasks file or model, find name corresponding to passed in index  */
std::string InverseKinematicsSolverExt::getDistanceSensorNameForIndex(
    int dsensorIndex) const {
  return _distanceAssemblyCondition->getDSensorName(
      SimTK::DistanceSensors::DSensorIx(dsensorIndex));
}
...
void InverseKinematicsSolverExt::setupMarkersGoal(SimTK::State &s) {
  ...
  // Setup markers goals
  // Get lists of all markers by names and corresponding weights from the
  // MarkersReference
  const SimTK::Array_<SimTK::String> &markerNames =
      _markersReference->getNames();
  ...
  for (unsigned int i = 0; i < markerNames.size(); ++i) {
    ...
    if (index >= 0) {
      ...
      _markerAssemblyCondition->addMarker(marker.getName(), mobod,
                                          X_BF * marker.get_location(),
                                          markerWeights[i]);
    }
  }
...
void InverseKinematicsSolverExt::setupOrientationsGoal(SimTK::State &s) {
  ...
  std::unique_ptr<SimTK::OrientationSensors> condOwner(
      new SimTK::OrientationSensors());
  ...
  for (const auto &modelFrame : onFrames) {
    const std::string &modelFrameName = modelFrame.getName();
    auto found =
        std::find(osensorNames.begin(), osensorNames.end(), modelFrameName);
    if (found != osensorNames.end()) {
      int index = (int)std::distance(osensorNames.begin(), found);
      _orientationAssemblyCondition->addOSensor(
          modelFrameName, modelFrame.getMobilizedBodyIndex(),
          modelFrame.findTransformInBaseFrame().R(), orientationWeights[index]);
    }
  }
...
void InverseKinematicsSolverExt::setupDistancesGoal(SimTK::State &s) {
  ...
  std::unique_ptr<SimTK::DistanceSensors> condOwner(
      new SimTK::DistanceSensors());
  ...
  for (const auto &imu : imus) {
    ...
    // auto found = std::find(dsensorNames.begin(), dsensorNames.end(),
    // modelFrameName);
    ...
    if (found != dsensorNames.end()) {
      ...
      for (const auto &imu2 : imus) {
        ...
        if (found2 != dsensorNames.end()) {
          ...
          const PhysicalFrame &imu_frame =
              imu.getSocket<PhysicalFrame>("frame").getConnectee();
          const PhysicalFrame &imu_frame_2 =
              imu2.getSocket<PhysicalFrame>("frame").getConnectee();
          const auto weight = distanceWeights[index];
          std::cout << "adding sensor: " << *found2
                    << " with weight: " << weight << std::endl;
          _distanceAssemblyCondition->addDSensor(
              *found2, imu_frame.getMobilizedBodyIndex(),
              imu_frame.findTransformInBaseFrame().p(),
              imu_frame_2.getMobilizedBodyIndex(),
              imu_frame_2.findTransformInBaseFrame().p(), weight);
        }
      }
    }
  }
...
/* Internal method to update the time, reference values and/or their weights
   based on the state */
void InverseKinematicsSolverExt::updateGoals(SimTK::State &s) {
  ...
  if (_advanceTimeFromReference) {
    ...
    if (_orientationsReference && _orientationsReference->getNumRefs() > 0) {
      ...
      nextTime =
          _orientationsReference->getNextValuesAndTime(orientationValues);
      s.setTime(nextTime);
