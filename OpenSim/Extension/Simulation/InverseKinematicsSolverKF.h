#ifndef OPENSIM_INVERSE_KINEMATICS_SOLVER_KF_H_
#define OPENSIM_INVERSE_KINEMATICS_SOLVER_KF_H_
/* -------------------------------------------------------------------------- *
 *                    OpenSim:  InverseKinematicsSolverKF.h                   *
 * -------------------------------------------------------------------------- *
 ...
 * values. The InverseKinematicsSolverKF provides the option to convert the
 ...
 * solution.
 *
 * The InverseKinematicsSolverKF objective:
 * \f[
 ...
 * @author Ajay Seth
 */
class OSIMSIMULATION_API InverseKinematicsSolverKF: public AssemblySolverExt
{
//==============================================================================
// METHODS
//==============================================================================
public:
    ...
    // CONSTRUCTION
    //--------------------------------------------------------------------------
    virtual ~InverseKinematicsSolverKF() {}
    // No need for copy constructor or operator
    InverseKinematicsSolverKF(const InverseKinematicsSolverKF& other)            = delete;
    InverseKinematicsSolverKF& operator=(const InverseKinematicsSolverKF& other) = delete;

    InverseKinematicsSolverKF(const Model& model, 
    ...
    InverseKinematicsSolverKF(const Model& model,
            ...
            SimTK::Array_<CoordinateReference>& coordinateReferences,
            double constraintWeight = SimTK::Infinity): 
        InverseKinematicsSolverKF(model,
    ...
    int getNumDistanceSensorsInUse() const;
    ...
    // DISTANCE
      /** Compute and return an orientation sensor's spatial orientation in the
    ground frame, given the o-sensor's name. */
    SimTK::Real computeCurrentSensorDistance(const std::string& dsensorName);
    /** Compute and return an orientation sensor's spatial orientation in the
    ground frame, given the o-sensor's index. */
    SimTK::Real computeCurrentSensorDistance(int dsensorIndex);
    /** Compute and return the spatial orientations of all o-sensors, expressed in
    the ground frame. */
    void computeCurrentSensorDistances(
        SimTK::Array_<SimTK::Real>& dsensorDistances);

            /** Compute and return the orientation error between the model orientation
    sensor and its observation, given the o-sensor's name. */
    double computeCurrentDistanceError(const std::string& dsensorName);
    /** Compute and return the orientation error between the model orientation
    sensor and its observation, given the o-sensor's index. */
    double computeCurrentDistanceError(int dsensorIndex);
    /** Compute all the orientation errors between the model orientation
    sensors and their observations. */
    void computeCurrentDistanceErrors(SimTK::Array_<double>& dsensorErrors);

    /** Distance sensor locations and errors may be computed in an order that
    may be different from tasks file or listed in the model. Return the 
    corresponding orientation sensor name for an index in the list of
    orientations returned by the solver. */
    std::string getDistanceSensorNameForIndex(int dsensorIndex) const;
...
private:
    ...
    void setupOrientationsGoal(SimTK::State &s);
        ...
        /** Define and apply Orientations of Frames to be tracked to the
        assembly problem. */
    void setupDistancesGoal(SimTK::State &s);
    ...
    // The distance reference values and weightings
    std::shared_ptr<DistancesReference> _distancesReference;
    ...
    // DistanceSensors collectively form a single assembly condition for
    // the SimTK::Assembler and the memory is managed by the Assembler
    SimTK::ReferencePtr<SimTK::DistanceSensors> _distanceAssemblyCondition;
...
//=============================================================================
};  // END of class InverseKinematicsSolverKF
//=============================================================================
} // namespace

#endif // OPENSIM_INVERSE_KINEMATICS_SOLVER_KF_H_
