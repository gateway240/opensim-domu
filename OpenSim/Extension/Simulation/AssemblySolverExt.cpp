 ...
 * Author(s): Alexander Beattie, Ajay Seth                                    *
 *                                                                            *
...
AssemblySolverExt::AssemblySolverExt
        ...
        p != _coordinateReferencesp.end(); p++) 
    {
        if(p){
            ...
            // Otherwise an error if the coordinate does not exist for this model
                ...
                throw(Exception("AssemblySolverExt: Model does not contain coordinate "+p->getName()+"."));
            }
        }
    }
}

void AssemblySolverExt::setAccuracy(double accuracy)
{
    _accuracy = accuracy;
...
void AssemblySolverExt::setupGoals(SimTK::State &s)
{
    // wipe-out the previous SimTK::Assembler
    _assembler.reset(new SimTK::AssemblerExt(getModel().getMultibodySystem()));
...
/* Once a set of coordinates has been specified its target value can
    be updated directly */
void AssemblySolverExt::updateCoordinateReference(const std::string &coordName, double value, double weight)
{
    SimTK::Array_<CoordinateReference>::iterator p;
...
void AssemblySolverExt::updateGoals(SimTK::State &s)
{
    unsigned int nqrefs = _coordinateReferencesp.size();
 ...
 */
void AssemblySolverExt::assemble(SimTK::State &state)
{
    ...
    {
        std::string msg = "AssemblySolverExt::assemble() Failed: ";
        msg += ex.what();
        throw Exception(msg);
    }
}
...
void AssemblySolverExt::track(SimTK::State &s)
{
    ...
    if(_assembler && _assembler->isInitialized()){
        ...
        throw Exception(
            "AssemblySolverExt::track() failed: assemble() must be called first.");
    }
    ...
    {
        log_info( "AssemblySolverExt::track() attempt Failed: {}", ex.what());
        throw Exception("AssemblySolverExt::track() attempt failed.");
    }
}

// Addition for the Kalman Smoother
void AssemblySolverExt::setState(SimTK::State &s)
{
	...
	if(_assembler && _assembler->isInitialized()){}
		...
		throw Exception("AssemblySolverExt::setState() failed: assemble() must be called first.");
	}
    ...
    {
		std::cout << "AssemblySolverExt::setState() attempt Failed: " << ex.what() << std::endl;
		throw Exception("AssemblySolverExt::setState() attempt failed.");
    }
}

SimTK::Matrix AssemblySolverExt::calcCurrentJacobian(SimTK::State &s)
{
    if(_assembler && _assembler->isInitialized()){}
    else{
		throw Exception("AssemblySolverExt::setState() failed: assemble() must be called first.");
	}
    return _assembler->calcCurrentJacobian(s);
}

SimTK::Vector AssemblySolverExt::calcCurrentGradient(SimTK::State &s)
{
    if(_assembler && _assembler->isInitialized()){}
    else{
		throw Exception("AssemblySolverExt::setState() failed: assemble() must be called first.");
	}
    return _assembler->calcCurrentGradient(s);
}

const SimTK::AssemblerExt& AssemblySolverExt::getAssembler() const
{
    OPENSIM_THROW_IF(!_assembler, Exception,
        "AssemblySolverExt::getAssembler() has no underlying Assembler to return.\n"
        "AssemblySolverExt::setupGoals() must be called first.");
    return *_assembler;
}

SimTK::AssemblerExt& AssemblySolverExt::updAssembler()
{
    OPENSIM_THROW_IF(!_assembler, Exception,
        "AssemblySolverExt::updAssembler() has no underlying Assembler to return.\n"
        "AssemblySolverExt::setupGoals() must be called first.");
    return *_assembler;
}
