...
MarkerPair::MarkerPair() :
    _distance(_distanceProp.getValueDbl()),
...
/**
 */
MarkerPair::MarkerPair(const std::string &aName1, const std::string &aName2, const double &distance) :
    _distance(_distanceProp.getValueDbl()),
    _markerNames(_markerNamesProp.getValueStrArray())
{
    setNull();
...
void MarkerPair::copyData(const MarkerPair &aMarkerPair)
{
    ...
    _distance = aMarkerPair._distance;
}
...
void MarkerPair::setupProperties()
{
    ...
    _distanceProp.setComment("Distance between two markers measured manually.");
    _distanceProp.setName("distance");
    _propertySet.append(&_distanceProp);
}
