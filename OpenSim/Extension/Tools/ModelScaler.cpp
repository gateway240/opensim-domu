...
double ModelScaler::computeMeasurementScaleFactor(const SimTK::State& s, const Model& aModel, const MarkerData& aMarkerData, const Measurement& aMeasurement) const
{
    ...
    for(int i=0; i<aMeasurement.getNumMarkerPairs(); i++) {
        ...
        double experimentalLength = pair.getDistance();
        if (!experimentalLength) {
            experimentalLength = takeExperimentalMarkerMeasurement(aMarkerData, name1, name2, aMeasurement.getName());
        }
        // std::cout << "Name1: " << name1 << " Name2: " << name2 << " Distance: " <<  pair.getDistance() << std::endl;
        double modelLength = takeModelMeasurement(s, aModel, name1, name2, aMeasurement.getName());
        if(SimTK::isNaN(modelLength) || SimTK::isNaN(experimentalLength)) {
            // TODO: Fix so that it doesn't return NAN for 1 missing marker set
            continue;
        }
    ...
    scaleFactor /= validMarkerPairs;
    log_info("\toverall scale factor = {}", scaleFactor);
    return validMarkerPairs ? scaleFactor : SimTK::NaN;
}
