...
#include <OpenSim/Extension/Simulation/InverseKinematicsSolverExt.h>

using namespace OpenSim;
using namespace SimTK;
using namespace std;

DistanceInverseKinematicsTool::DistanceInverseKinematicsTool()
...
void DistanceInverseKinematicsTool::constructProperties() {
  ...
  constructProperty_distances_file("");
  DistanceWeightSet distanceWeights;
  constructProperty_distance_weights(distanceWeights);
}
/**
void DistanceInverseKinematicsTool::
    previewExperimentalData(const TimeSeriesTableVec3& markers,
                const TimeSeriesTable_<SimTK::Rotation>& distances) const
{
    Model previewWorld;
...
*/
void DistanceInverseKinematicsTool::runInverseKinematicsWithDistancesFromFile(
    Model &model, const std::string &orientationsFileName,
    const std::string &distancesFileName, bool visualizeResults) {

  // Ideally if we add a Reporter, we also remove it at the end for good hygiene
  // but at the moment there's no interface to remove Reporter so we'll reuse
  // one if exists
  ...
  for (auto &coord : coordinates) {
    ikReporter->updInput("inputs").connect(coord.getOutput("value"),
                                           coord.getName());
  ...
  if (!reuse_reporter) {
   ...
   // DISTANCES
  TimeSeriesTable_<double> distTable(distancesFileName);
  ...
  DistancesReference dRefs(distTable, &get_distance_weights());
  ...
  SimTK::Array_<string> names = dRefs.getNames();
  ...
  InverseKinematicsSolverExt ikSolver(
      model, nullptr, std::make_shared<OrientationsReference>(oRefs),
      std::make_shared<DistancesReference>(dRefs), coordinateReferences);
  ikSolver.setAccuracy(accuracy);

  auto &times = oRefs.getTimes();
  std::shared_ptr<TimeSeriesTable> modelDistanceErrors(
      get_report_errors() ? new TimeSeriesTable() : nullptr);
  s0.updTime() = times[0];
  ikSolver.assemble(s0);
  // Create place holder for distance errors, populate based on user pref.
  // according to report_errors property
  int nos = ikSolver.getNumDistanceSensorsInUse();
  SimTK::Array_<double> distanceErrors(nos, 0.0);

  if (get_report_errors()) {
    SimTK::Array_<string> labels;
    for (int i = 0; i < nos; ++i) {
      labels.push_back(ikSolver.getDistanceSensorNameForIndex(i));
    }
    modelDistanceErrors->setColumnLabels(labels);
    modelDistanceErrors->updTableMetaData().setValueForKey<string>(
        "name", "DistanceErrors");
    ikSolver.computeCurrentDistanceErrors(distanceErrors);
  }
  if (visualizeResults) {
  ...
  for (auto time : times) {
    ...
    if (get_report_errors()) {
      ikSolver.computeCurrentDistanceErrors(distanceErrors);
      modelDistanceErrors->appendRow(s0.getTime(), distanceErrors);
    }
    if (visualizeResults)
      model.getVisualizer().show(s0);
    else
  ...
  if (!resultsDir.empty()) {
    ...
    if (outName.empty()) {
      ...
      SimTK::Pathname::deconstructPathname(distancesFileName, isAbsolutePath,
    ...
    log_info("Wrote IK with Distance tracking results to: '{}'.",
             fullOutputFilename);
    if (get_report_errors()) {
      STOFileAdapter_<double>::write(*modelDistanceErrors,
                                     outName + "_distanceErrors.sto");
    }
  } else
    log_info("DistanceInverseKinematicsTool: No output files were generated, "
...
// main driver
bool DistanceInverseKinematicsTool::run(bool visualizeResults) {
  if (_model.empty()) {
    _model.reset(new Model(get_model_file()));
  }

  runInverseKinematicsWithDistancesFromFile(
      *_model, get_orientations_file(), get_distances_file(), visualizeResults);

  return true;
}

TimeSeriesTable_<SimTK::Vec3>
DistanceInverseKinematicsTool::loadMarkersFile(const std::string &markerFile) {
