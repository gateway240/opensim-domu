/* -------------------------------------------------------------------------- *
 *                       OpenSim:  DistanceKinematics.cpp                        *
 * -------------------------------------------------------------------------- *
...
// CONSTANTS
//=============================================================================
const int DistanceKinematics::NAME_LENGTH = DistanceKinematicsNAME_LENGTH;
const int DistanceKinematics::BUFFER_LENGTH = DistanceKinematicsBUFFER_LENGTH;
 ...
 * Destructor.
 */
DistanceKinematics::~DistanceKinematics()
{
    deleteStorage();
}
//_____________________________________________________________________________
/**
 * Construct an DistanceKinematics instance for recording the kinematics of
 ...
 */
DistanceKinematics::DistanceKinematics(Model *aModel) :
Analysis(aModel),
_body(NULL),
 ...
 */
DistanceKinematics::DistanceKinematics(const std::string &aFileName):
 ...
 *
 */
DistanceKinematics::DistanceKinematics(const DistanceKinematics &aDistanceKinematics):
Analysis(aDistanceKinematics),
_body(aDistanceKinematics._body),
_relativeToBody(aDistanceKinematics._relativeToBody),
_bodyName(_bodyNameProp.getValueStr()),
_point(_pointProp.getValueDblVec()),
_pointName(_pointNameProp.getValueStr()),
_relativeToBodyName(_relativeToBodyNameProp.getValueStr())
 ...
 */
DistanceKinematics& DistanceKinematics::
operator=(const DistanceKinematics &aDistanceKinematics)
{
    // BASE CLASS
    Analysis::operator=(aDistanceKinematics);
    _body = aDistanceKinematics._body;
    _relativeToBody = aDistanceKinematics._relativeToBody;
    _point = aDistanceKinematics._point;
    _pointName = aDistanceKinematics._pointName;
    _bodyName = aDistanceKinematics._bodyName;
    _relativeToBodyName = aDistanceKinematics._relativeToBodyName;
...
void DistanceKinematics::setModel(Model& model)
{
 ...
 * @param aBody Body ID
 */
void DistanceKinematics::setBody(const PhysicalFrame* aBody)
{
    // CHECK
    if (aBody==NULL) {
        log_warn("DistanceKinematics.setBody: null body pointer.");
        _body = NULL;
        return;
    }
    ...
    log_info("DistanceKinematics.setBody: set body to {}.", _bodyName);
}
void DistanceKinematics::setRelativeToBody(const PhysicalFrame* aBody)
{
    // CHECK
    if (aBody==NULL) {
        log_warn("DistanceKinematics.setRelativeToBody: null body pointer.");
        _relativeToBody = NULL;
        return;
    }
    ...
    log_info("DistanceKinematics.setRelativeToBody: set relative-to body to {}.",
            _relativeToBodyName);
}
 ...
 * @return Body pointer
 */
const PhysicalFrame* DistanceKinematics::getBody() const
{
    return(_body);
}

const PhysicalFrame* DistanceKinematics::getRelativeToBody() const
{
 ...
 * @param aName Name for the point.
 */
const std::string &DistanceKinematics::
getPointName()
{
    return(_pointName);
}
...
record(const SimTK::State& s)
{
    ...
    if(_relativeToBody && _body){
        SimTK::Vec3 pointA_inGround = ground.findStationLocationInAnotherFrame(s, vec, *_body);
        SimTK::Vec3 pointB_inGround = ground.findStationLocationInAnotherFrame(s, vec, *_relativeToBody);
        dist = (pointA_inGround - pointB_inGround).norm();
...
end( const SimTK::State& s)
{
    if(!proceed()) return(0);
    record(s);
    log_info("DistanceKinematics.end: Finalizing analysis {}.", getName());
    return 0 ;
}
